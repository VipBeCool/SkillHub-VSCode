...
<body>
    ...
    <script>
        ...
        // 存储原始和翻译的 markdown html (safeJsonStringify 防提前闭合)
        const rawMarkdown = ${safeJsonStringify(fileContent)};
        const originalHtml = ${safeJsonStringify(fileContent ? md.render(fileContent) : '<p>No content available.</p>')};
